...
const welcome = () => {
  ...
  console.log(
    ` ${user}, bienvenido a la pagina web de la aerolinea Programadores del Aire, le informamos que los vuelos del dia de hoy son los siguientes:  `
  );
};
const welcomeTwo = () => {
  let user = prompt("Buenos dias, ¿Cual es su nombre?");
  user = user.toUpperCase();
  console.log(
    ` ${user}, bienvenido a la pagina web de la aerolinea Programadores del Aire, este es el listado de vuelos luego de las modificaciones realizadas:   `
  );
};
...
const showFlights = () => {
  flights.forEach((flight) => {
    if (flight.scale === false) {
      console.log(
        `---> El vuelo Numero ${flight.id} con origen: ${flight.from}, y destino: ${flight.to} tiene un coste de ${flight.cost}€ y no realiza ninguna escala.\n`
      );
    } else {
      console.log(
        `---> El vuelo Numero ${flight.id} con origen: ${flight.from}, y destino: ${flight.to} tiene un coste de ${flight.cost}€ y es un vuelo con escalas programadas.\n`
      );
    }
  });
};
...
const averageFlights = () => {
  ...
  console.log(
    `\nProgramadores del Aire le informa que el precio medio de los vuelos del dia de hoy es de ${
      sum / flights.length
    }€.`
  );
};
...
const askPrice = () => {
  // devuelve undefine si la llamo asi: console.log(askPrice())
  const price = +prompt(
    `Por favor ingrese el presupuesto para su vuelo en euros`
  );
  if (price === null || Number.isNaN(+price) || price === "" || price === 0) {
    return askPrice();
  } else {
    return price;
  }
...
const lookPrice = (price) => {
  ...
  flights.forEach((flight) => {
    if (flight.cost <= price) {
      console.log(
        `El vuelo con codigo ${flight.id} con salida de ${flight.from} y llegada a ${flight.to} tiene un coste de ${flight.cost}`
      );
    }
  });
};
...
const whatToDo = () => {
  ...
  if (toDo === null) {
    ...
    toDo.toUpperCase() !== "CREAR" &&
    toDo.toUpperCase() !== "ELIMINAR"
  ) {
...
const adminToDo = () => {
  ...
  if (toDoResult === "CREAR") {
    const newFlight = {};
    newFlight.id = +prompt(
      "Intruduzca un nuevo id para el vuelo a crear, verifique que el id no este en uso."
    );
